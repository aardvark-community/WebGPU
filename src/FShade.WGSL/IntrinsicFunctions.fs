...
module WGSLIntrinsicInput =
    let name (input : WGSLIntrinsicInput) =
        match input with
        | WGSLIntrinsicInput.GlobalId -> "global_id"
        | WGSLIntrinsicInput.LocalId -> "local_id"
        | WGSLIntrinsicInput.LocalIndex -> "local_index"
        | WGSLIntrinsicInput.WorkGroupSize -> "work_group_size"
        | WGSLIntrinsicInput.NumWorkGroups -> "num_work_groups"
        | WGSLIntrinsicInput.WorkGroupId -> "work_group_id"
        | _ -> failwith "Invalid WGSL intrinsic input"
...
module IntrinsicFunctions =
    let (|IntrinsicFunction|_|) : MethodInfo -> Option<CIntrinsic> =
        MethodTable.ofList [
            ...
            CIntrinsic.tagged "vec2i(sign({0}))", [
            ...
            CIntrinsic.tagged "vec3i(sign({0}))", [
            ...
            CIntrinsic.tagged "vec4i(sign({0}))", [
            ...
            CIntrinsic.tagged "min(vec2({0}), {1})", [
            ...
            CIntrinsic.tagged "min(vec3({0}), {1})", [
            ...
            CIntrinsic.tagged "min(vec4({0}), {1})", [
            ...
            CIntrinsic.tagged "min({0}, vec2({1}))", [
                exactly <@ Fun.Min : V2i   * int32   -> V2i @>
                exactly <@ Fun.Min : V2l   * int64   -> V2l @>
                exactly <@ Fun.Min : V2f   * float32 -> V2f @>
                exactly <@ Fun.Min : V2d   * float   -> V2d @>
            ...
            CIntrinsic.tagged "min({0}, vec3({1}))", [
                exactly <@ Fun.Min : V3i   * int32   -> V3i @>
                exactly <@ Fun.Min : V3l   * int64   -> V3l @>
                exactly <@ Fun.Min : V3f   * float32 -> V3f @>
                exactly <@ Fun.Min : V3d   * float   -> V3d @>
            ...
            CIntrinsic.tagged "min({0}, vec4({1}))", [
                exactly <@ Fun.Min : V4i   * int32   -> V4i @>
                exactly <@ Fun.Min : V4l   * int64   -> V4l @>
                exactly <@ Fun.Min : V4f   * float32 -> V4f @>
                exactly <@ Fun.Min : V4d   * float   -> V4d @>
            ...
            CIntrinsic.tagged "max(vec2({0}), {1})", [
            ...
            CIntrinsic.tagged "max(vec3({0}), {1})", [
            ...
            CIntrinsic.tagged "max(vec4({0}), {1})", [
            ...
            CIntrinsic.tagged "max({0}, vec2({1}))", [
                exactly <@ Fun.Max : V2i   * int32   -> V2i @>
                exactly <@ Fun.Max : V2l   * int64   -> V2l @>
                exactly <@ Fun.Max : V2f   * float32 -> V2f @>
                exactly <@ Fun.Max : V2d   * float   -> V2d @>
            ...
            CIntrinsic.tagged "max({0}, vec3({1}))", [
                exactly <@ Fun.Max : V3i   * int32   -> V3i @>
                exactly <@ Fun.Max : V3l   * int64   -> V3l @>
                exactly <@ Fun.Max : V3f   * float32 -> V3f @>
                exactly <@ Fun.Max : V3d   * float   -> V3d @>
            ...
            CIntrinsic.tagged "max({0}, vec4({1}))", [
                exactly <@ Fun.Max : V4i   * int32   -> V4i @>
                exactly <@ Fun.Max : V4l   * int64   -> V4l @>
                exactly <@ Fun.Max : V4f   * float32 -> V4f @>
                exactly <@ Fun.Max : V4d   * float   -> V4d @>
            ...
            CIntrinsic.tagged "i32(round(mix({1}, {2}, {0})))", [
            ...
            CIntrinsic.tagged "i32(round(mix({0}, {1}, {2})))", [
            ...
            CIntrinsic.tagged "u32(mix({1}, {2}, {0}) + 0.5)", [
            ...
            CIntrinsic.tagged "u32(mix({0}, {1}, {2}) + 0.5)", [
            ...
            CIntrinsic.tagged "vec2i(round(mix({1}, {2}, {0})))", [
            ...
            CIntrinsic.tagged "vec2i(round(mix({0}, {1}, {2})))", [
            ...
            CIntrinsic.tagged "vec3i(round(mix({1}, {2}, {0})))", [
            ...
            CIntrinsic.tagged "vec3i(round(mix({0}, {1}, {2})))", [
            ...
            CIntrinsic.tagged "vec4i(round(mix({1}, {2}, {0})))", [
            ...
            CIntrinsic.tagged "vec4i(round(mix({0}, {1}, {2})))", [
            ...
            CIntrinsic.tagged "fma({0}, vec2i({1}), {2})", [
                exactly <@ madd : V2i -> int32 -> V2i -> _ @>
            ]

            CIntrinsic.tagged "fma({0}, vec3i({1}), {2})", [
                exactly <@ madd : V3i -> int32 -> V3i -> _ @>
            ]

            CIntrinsic.tagged "fma({0}, vec4i({1}), {2})", [
            ...
            CIntrinsic.simple "countOneBits", [
            ...
            CIntrinsic.simple "firstLeadingBit", [
            ...
            CIntrinsic.simple "firstTrailingBit", [
            ...
            CIntrinsic.simple "pack2x16unorm", [ exactly <@ packUnorm2x16 @> ]
            CIntrinsic.simple "pack2x16snorm", [ exactly <@ packSnorm2x16 @> ]
            CIntrinsic.simple "pack4x8unorm", [ exactly <@ packUnorm4x8 @> ]
            CIntrinsic.simple "pack4x8snorm", [ exactly <@ packSnorm4x8 @> ]

            CIntrinsic.simple "unpack2x16unorm", [ exactly <@ unpackUnorm2x16 @> ]
            CIntrinsic.simple "unpack2x16snorm", [ exactly <@ unpackSnorm2x16 @> ]
            CIntrinsic.simple "unpack4x8unorm", [ exactly <@ unpackUnorm4x8 @> ]
            CIntrinsic.simple "unpack4x8snorm", [ exactly <@ unpackSnorm4x8 @> ]

            CIntrinsic.simple "dpdx", [ generic <@ ddx : float -> _ @> ]
            CIntrinsic.simple "dpdy", [ generic <@ ddy : float -> _ @> ]
            CIntrinsic.simple "dpdxFine", [ generic <@ ddxFine : float -> _ @> ]
            CIntrinsic.simple "dpdyFine", [ generic <@ ddyFine : float -> _ @> ]
            CIntrinsic.simple "dpdxCoarse", [ generic <@ ddxCoarse : float -> _ @> ]
            CIntrinsic.simple "dpdyCoarse", [ generic <@ ddyCoarse : float -> _ @> ]
            ...
            CIntrinsic.tagged WGSLIntrinsicInput.NumWorkGroups, [ exactly <@ getWorkGroupCount @> ]
            CIntrinsic.tagged WGSLIntrinsicInput.WorkGroupId, [ exactly <@ getWorkGroupId @> ]
            CIntrinsic.tagged WGSLIntrinsicInput.LocalId, [ exactly <@ getLocalId @> ]
            CIntrinsic.tagged WGSLIntrinsicInput.GlobalId, [ exactly <@ getGlobalId @> ]
            CIntrinsic.tagged WGSLIntrinsicInput.LocalIndex, [ exactly <@ getLocalIndex @> ]
            CIntrinsic.tagged WGSLIntrinsicInput.WorkGroupSize, [ exactly <@ getWorkGroupSize @> ]
        ...
        | Method(name, (((SamplerType(dim, isArray, isShadow, isMS, valueType)) as sam ::_) as args)) when mi.DeclaringType = sam ->
            ...
            let fetchArgs() =
                let consumedArgs, sampleArgs =
                    match isArray with
                        | true -> 3, sprintf "{0}, vec%d({1}, {2})" (coordComponents + 1)
                        | false -> 2, "{0}, {1}"
            ...
            let functionName =
                ...
                | "Sample" -> sprintf "textureSample%s(%s)" suffix (sampleArgs true)
                | "SampleOffset" -> sprintf "textureSample%s(%s)" suffix (sampleArgs true)
                | "SampleLevel" -> sprintf "textureSample%sLevel(%s)" suffix (sampleArgs true)
                | "SampleLevelOffset" -> sprintf "textureSample%sLevel(%s)" suffix (sampleArgs true)
                | "SampleGrad" -> sprintf "textureSampleGrad(%s)" (sampleArgs true)
                | "Gather" -> sprintf "textureGather%s(%s)" suffix (sampleArgs true)
                | "GatherOffset" -> sprintf "textureGather%s(%s)" suffix (sampleArgs true)

                | "Read" -> sprintf "textureLoad(%s)" (fetchArgs())
                | "get_Item" -> sprintf "textureLoad(%s)" (fetchArgs())

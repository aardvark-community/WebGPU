...
EMSCRIPTEN_KEEPALIVE uint32_t wgpu_get_num_live_objects(void);
...
EMSCRIPTEN_KEEPALIVE void wgpu_object_destroy(WGpuObjectBase wgpuObject);

// Deinitializes all initialized WebGPU objects.
EMSCRIPTEN_KEEPALIVE void wgpu_destroy_all_objects(void);

#ifdef __EMSCRIPTEN__
// Initializes a WebGPU rendering context to a canvas by calling canvas.getContext('webgpu').
EMSCRIPTEN_KEEPALIVE WGpuCanvasContext wgpu_canvas_get_webgpu_context(const char *canvasSelector NOTNULL);

// Initializes a WebGPU rendering context to the given Offscreen Canvas.
EMSCRIPTEN_KEEPALIVE WGpuCanvasContext wgpu_offscreen_canvas_get_webgpu_context(OffscreenCanvasId id);
#elif defined (_WIN32)
...
EMSCRIPTEN_KEEPALIVE WGPU_BOOL wgpu_is_valid_object(WGpuObjectBase obj);
// Set a human-readable label for the given WebGPU object. Pass an empty string "" to clear a label.
EMSCRIPTEN_KEEPALIVE void wgpu_object_set_label(WGpuObjectBase obj, const char *label NOTNULL);
...
EMSCRIPTEN_KEEPALIVE int wgpu_object_get_label(WGpuObjectBase obj, char *dstLabel NOTNULL, uint32_t dstLabelSize);
...
EMSCRIPTEN_KEEPALIVE WGPU_BOOL navigator_gpu_request_adapter_async(const WGpuRequestAdapterOptions *options NOTNULL, WGpuRequestAdapterCallback adapterCallback, void *userData);
